...
ALTER TABLE orders
    RENAME COLUMN total_amoutn to total_amount
...
SELECT * FROM employees as E
 ...
 --2. Group By Department with Average Salary
  SELECT department_name, avg(salary)::NUMERIC(10, 2) AS avg_salary FROM employees
...
SELECT department_name,  avg(salary)::NUMERIC(10, 0) as avg_salary  FROM departments
    INNER JOIN employees USING(department_id)
        GROUP BY department_name
...
SELECT extract(year from hire_date::date) as year, count(employee_name) FROM employees
    GROUP BY year


--6.  Find customers who have placed more than 2 orders and calculate the total amount spent by   each of these customers.
SELECT customer_id, sum(total_amount) as total_spent, count(order_id) as total_order FROM orders
 GROUP BY customer_id
    HAVING count(order_id) > 2;


--7 Find the total amount of orders placed each month in the year 2022.
SELECT extract(month from order_date ) as month_of_2022, sum(total_amount) as total_amount FROM orders
